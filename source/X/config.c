...
GtkWidget       *separator;
GtkWidget       *window,*pinwin;
GtkWidget       *lvbox,*rvbox,*bbox;
GtkWidget       *scrolled_window;
GtkWidget       *frame;
GtkWidget       *gtklist;
GtkWidget       *cl_button,*add_button,*del_button,*fname_entry,*sname_entry,*uid_entry,*last_entry;
GtkWidget       *list_item,*fnm_label,*snm_label,*uid_label,*last_label;
GtkWidget       *master_hbox;
GtkWidget *fnamebox,*snamebox,*uidbox,*lastbox,*pinbox,*pin_button,*log_button,*pixmapwid,
  *deptbox, *dept_entry, *dp_label, *pvbox, *lok_button,*pin_ok_button;
GdkPixmap *pixmap;
GdkBitmap *mask;
GtkStyle *style;
GtkWidget *status_bar;
GtkWidget *master_vbox,*pin_entry1,*pin_entry2,*pin_vbox,*pin_label,*pinbuttonbox,*pin_c_button;
...
{                                  
    ...
    dlist=NULL;
    i=0;
    while(db_store[i].uid[0] != 'X'){
      char name[60];
      sprintf(name,"%s %s",db_store[i].fname,db_store[i].sname);
    ...
    */
    ...
    /*gtk_signal_connect_object(GTK_OBJECT(del_button),
			      "clicked",
    ...
    /* a third vbox for picture and lockout */
    /*pvbox = gtk_vbox_new(FALSE, 5);
    gtk_container_border_width(GTK_CONTAINER(pvbox), 5);
    gtk_box_pack_start (GTK_BOX (master_hbox), pvbox, TRUE, TRUE, TRUE);
    ...
    pixmap = gdk_pixmap_create_from_xpm( window->window,  &mask,
				       ...
				       "/home/sysd7/images/duncan.xpm");*/
    /* a pixmap widget to contain the pixmap */
    /*pixmapwid = gtk_pixmap_new( pixmap, mask );
    gtk_box_pack_start (GTK_BOX (pvbox), pixmapwid, TRUE, TRUE, TRUE);
    ...
    gtk_statusbar_push(status_bar,context_id,"Click a user to see details.");
...
void user_selected (GtkWidget *gtklist,gpointer func_data)
{
   ...
   */
	...
	if( (item->photo[0] !='%') && (access(item->photo,F_OK))){
	printf("xpm found\n");
	style = gtk_widget_get_style( window );
	pixmap2 = gdk_pixmap_create_from_xpm( window->window,  &mask,
	&style->bg[GTK_STATE_NORMAL],
	item->photo);
	gtk_pixmap_set( (GtkPixmap *)pixmapwid,pixmap2,mask);
	} else {
	printf("File not found\n");
	style = gtk_widget_get_style( window );
	pixmap = gdk_pixmap_create_from_xpm( window->window,  &mask,
	&style->bg[GTK_STATE_NORMAL],
	"/home/sysd/source/X/james.xpm");
...
void enter_pin()
{
  ...
  /*  gtk_signal_connect(GTK_OBJECT(pinwin),
		     "destroy",
